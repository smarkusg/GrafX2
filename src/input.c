...
static int Handle_mouse_move(SDL_MouseMotionEvent * event)
{
  ...
  return Move_cursor_with_constraints(event->x / Pixel_width / render_scale_x,
                                      event->y / Pixel_height / render_scale_y);
}
...
#ifdef USE_SDL2
// We can throttle mouse motion handling to avoid jerky / lagging mouse movement
// See https://pulkomandy.tk/projects/GrafX2/ticket/136
          ...
          case SDL_WINDOWEVENT:
              switch(event.window.event)
              {
                case SDL_WINDOWEVENT_RESIZED: // change by external event (user or window manager)
                  ...
                  if (Resize_width < 320*render_scale_x)
                    Resize_width = 320*render_scale_x;
                  if (Resize_height < 200*render_scale_y)
                    Resize_height = 200*render_scale_y;
...
#ifdef USE_SDL2
              if (Config.Mouse_motion_debounce)
              {
                Uint32 ticks = SDL_GetTicks();
                if (SDL_TICKS_PASSED(ticks, next_mouse_motion_ticks)) {
                    user_feedback_required = Handle_mouse_move(&event.motion);
                    next_mouse_motion_ticks = ticks + Config.Mouse_motion_debounce;
                }
              }
              else
                user_feedback_required = Handle_mouse_move(&event.motion);
