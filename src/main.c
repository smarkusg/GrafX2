 ...
 *
 * Web for the project users is http://grafx2.eu/.
 *
...
int Init_program(int argc,char * argv[])
{
...
#ifdef __macosx__
  // use user folder on macOS
  extern char* getMacosUserFolder(void);
  ...
  render_scale_x = 1;
  render_scale_y = 1;
  envvar_val = getenv("GRAFX2_RENDER_SCALE");
  if (envvar_val != NULL) {
    char* s1 = strtok(envvar_val, ",");
    char* s2 = strtok(NULL, ",");
    render_scale_x = (int)strtol(s1, NULL, 10);
    if (render_scale_x < 1)
      render_scale_x = 1;

    if (s2 == NULL)
      render_scale_y = render_scale_x;
    else {
      render_scale_y = (int)strtol(s2, NULL, 10);
      if (render_scale_y < 1)
        render_scale_y = 1;
    }
  }
  ...
  *Paintbrush_sprite=1;
  ...
  if (Init_all_backup_lists(starting_image_mode, Main.image_width, Main.image_height)==0)
    Error(ERROR_MEMORY);
