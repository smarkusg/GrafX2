...
void GFX2_Set_mode(int *width, int *height, int fullscreen)
{
  ...
  render_dstrect.x = 0;
  render_dstrect.y = 0;
  render_dstrect.w = Screen_width * Pixel_width * render_scale_x;
  render_dstrect.h = Screen_height * Pixel_height * render_scale_y;
  if (Screen_SDL != NULL)
    SDL_FreeSurface(Screen_SDL);
  Screen_SDL = SDL_CreateRGBSurface(0, Screen_width * Pixel_width, Screen_height * Pixel_height, 8, 0, 0, 0, 0);
#endif
...
void Set_mouse_position(void)
{
  ...
  SDL_WarpMouseInWindow(NULL, Mouse_X*Pixel_width*render_scale_x, Mouse_Y*Pixel_height*render_scale_y);
#endif
}
...
int GFX2_MessageBox(const char * text, const char * caption, unsigned int type)
{
  ...
  CFStringRef text_ref = CFStringCreateWithCString(NULL, text, (unsigned int)strlen(text));
  CFStringRef caption_ref = CFStringCreateWithCString(NULL, caption, (unsigned int)strlen(text));
